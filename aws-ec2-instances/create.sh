...
Instance_Create() {
  ...
  if [ -z "${INSTANCE_EXISTS}" -o "$STATE" == "terminated"  ]; then
    aws ec2 run-instances --launch-template LaunchTemplateId=${LID},Version=${LVER}  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${COMPONENT}}, {Key=Project,Value=TODOAPP}]" | jq
  else
  ...
  aws route53 change-resource-record-sets --hosted-zone-id Z09973211FB946CEND2TK --change-batch file:///tmp/record.json
